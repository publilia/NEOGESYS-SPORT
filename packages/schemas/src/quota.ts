...
export const createQuotaSchema = z.object({
	...
	scadenza: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Formato data scadenza: YYYY-MM-DD"),

	tipoRateizzazione: z.enum(["unica", "mensile", "trimestrale", "semestrale"]).default("unica"),
...
export const emettiQuotaSchema = z.object({
	...
	dataEmissione: z
		.string()
		.regex(/^\d{4}-\d{2}-\d{2}$/, "Formato data emissione: YYYY-MM-DD")
		.optional(),
});
...
export const registraPagamentoSchema = z.object({
	...
	dataPagamento: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Formato data pagamento: YYYY-MM-DD"),
