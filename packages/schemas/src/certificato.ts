 ...
 */
const TIPI_CERTIFICATO = ["agonistico", "non_agonistico", "ludico_motoria"] as const;
...
export const createCertificatoSchema = z.object({
	...
	dataEmissione: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Formato data emissione: YYYY-MM-DD"),
	dataScadenza: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Formato data scadenza: YYYY-MM-DD"),
