...
export const createSocioSchema = z.object({
  ...
  dataNascita: z.string().regex(
    /^\d{4}-\d{2}-\d{2}$/,
    "Formato data nascita: YYYY-MM-DD",
  ),
  ...
  dataIscrizione: z.string().regex(
    /^\d{4}-\d{2}-\d{2}$/,
    "Formato data iscrizione: YYYY-MM-DD",
  ).optional(),
...
export const updateSocioSchema = createSocioSchema.partial().extend({
  stato: z
    .enum(["attivo", "sospeso", "cessato", "in_attesa"])
    .optional(),
});
