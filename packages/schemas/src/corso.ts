...
export const createCorsoSchema = z.object({
  ...
  dataInizio: z.string().regex(
    /^\d{4}-\d{2}-\d{2}$/,
    "Formato data inizio: YYYY-MM-DD",
  ),
  dataFine: z.string().regex(
    /^\d{4}-\d{2}-\d{2}$/,
    "Formato data fine: YYYY-MM-DD",
  ).optional(),

  quotaIscrizione: z.coerce.number().min(0).optional(),
  quotaMensile: z.coerce.number().min(0).optional(),

  colore: z.string().regex(/^#[0-9a-fA-F]{6}$/, "Colore deve essere un codice esadecimale").optional(),
...
export const updateCorsoSchema = createCorsoSchema.partial().extend({
  stato: z
    .enum(["attivo", "sospeso", "concluso", "programmato"])
    .optional(),
});
...
export const registraPresenzaSchema = z.object({
  ...
  presenze: z.array(
    z.object({
      ...
      oraIngresso: z.string().regex(/^\d{2}:\d{2}$/, "Formato ora: HH:MM").optional(),
      oraUscita: z.string().regex(/^\d{2}:\d{2}$/, "Formato ora: HH:MM").optional(),
    }),
