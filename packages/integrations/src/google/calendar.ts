import { CALENDAR_SCOPES, GoogleOAuthClient } from "./auth";
import type {
	Calendar,
	CalendarEvent,
	CalendarEventList,
...
export class GoogleCalendarService implements GoogleCalendarProvider {
	...
	async createEvent(calendarId: string, event: CreateCalendarEvent): Promise<CalendarEvent> {
		const raw = await this.request<Record<string, unknown>>(
	...
	async watchEvents(calendarId: string, webhookUrl: string): Promise<WatchResponse> {
	...
	async stopWatch(channelId: string, resourceId: string): Promise<void> {
		await this.request<void>(`${CALENDAR_API_BASE}/channels/stop`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
	...
	async syncEventiToCalendar(
		...
		for (const evento of eventi) {
			const colorId = evento.tipo ? (SPORT_COLOR_MAP[evento.tipo] ?? undefined) : undefined;
