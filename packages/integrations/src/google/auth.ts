...
export class GoogleAuthError extends Error {
	...
	constructor(message: string, statusCode: number, googleErrorCode?: string) {
		super(message);
...
export class GoogleOAuthClient {
	...
	async refreshToken(): Promise<string> {
		if (!this.config.refreshToken) {
			throw new GoogleAuthError("Refresh token mancante. Completare prima il flusso OAuth.", 401);
		}
