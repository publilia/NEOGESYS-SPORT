 ...
 */
export async function getSDIProvider(tenantId: string): Promise<SDIProvider> {
	...
	if (!integration) {
		throw new Error(`Nessuna integrazione SDI attiva trovata per il tenant ${tenantId}`);
	}
