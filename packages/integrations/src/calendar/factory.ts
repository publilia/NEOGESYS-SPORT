...
import type { OutlookEvent, OutlookCalendar } from "../microsoft/types";

// ─── Provider Detection ────────────────────────────────────────────────────

async function detectCalendarProvider(
  tenantId: string,
): Promise<CalendarProviderType> {
  const integrations = await db
    ...
    .where(
      and(
        ...
        inArray(tenantIntegrations.provider, [
          "google_calendar",
          "microsoft_calendar",
        ]),
  ...
  if (integrations.length === 0) {
    throw new Error(
      `Nessuna integrazione calendario attiva trovata per il tenant ${tenantId}. ` +
        "Configurare Google Calendar o Microsoft Calendar nelle impostazioni.",
    );
  }
...
function toUnifiedOutlookEvent(
  event: OutlookEvent,
  calendarId: string,
): UnifiedCalendarEvent {
  return {
