...
export async function getPaymentProvider(tenantId: string): Promise<PaymentProvider> {
	...
	if (!integration) {
		throw new Error(`Nessuna integrazione pagamento attiva trovata per il tenant ${tenantId}`);
	}
	...
	switch (providerType) {
		case "stripe":
			return new StripeProvider(credentials.secretKey ?? "", credentials.webhookSecret ?? "");
