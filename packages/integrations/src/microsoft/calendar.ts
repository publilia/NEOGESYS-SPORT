import { CALENDAR_SCOPES, MicrosoftAuthError, MicrosoftOAuthClient } from "./auth";
import type {
	CreateOutlookEvent,
	MicrosoftAuthConfig,
	OutlookCalendar,
...
export class OutlookCalendarService implements OutlookCalendarProvider {
	...
	async createEvent(event: CreateOutlookEvent, calendarId?: string): Promise<OutlookEvent> {
		const h = await this.headers();
	...
	async updateEvent(eventId: string, event: Partial<CreateOutlookEvent>): Promise<OutlookEvent> {
		const h = await this.headers();
	...
	async subscribeToChanges(webhookUrl: string, resource: string): Promise<Subscription> {
		const h = await this.headers();
	...
	async syncCorsiToCalendar(
		...
		for (const corso of corsi) {
			const categories = corso.disciplina ? [corso.disciplina] : [];
			...
			const event: CreateOutlookEvent = {
				...
				body: corso.descrizione ? { contentType: "text", content: corso.descrizione } : undefined,
	...
	async syncEventiToCalendar(
		...
		for (const evento of eventi) {
			const categories = evento.disciplina ? [evento.disciplina] : [];
