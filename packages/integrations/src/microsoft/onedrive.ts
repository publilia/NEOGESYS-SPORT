import { MicrosoftOAuthClient, ONEDRIVE_SCOPES, MicrosoftAuthError } from "./auth";
import type {
  MicrosoftAuthConfig,
  OneDriveFile,
  OneDriveFileList,
...
function mapDriveItem(item: Record<string, unknown>): OneDriveFile {
  return {
    ...
    mimeType: (item.file as Record<string, unknown> | undefined)?.mimeType as string ?? "application/octet-stream",
    ...
    parentReference: {
      ...
      driveId: (item.parentReference as Record<string, unknown> | undefined)?.driveId as string | undefined,
      path: (item.parentReference as Record<string, unknown> | undefined)?.path as string | undefined,
    },
...
export class OneDriveService implements OneDriveProvider {
  ...
  private async ensureTenantFolder(): Promise<string> {
    ...
    const getRes = await fetch(
      `${GRAPH_BASE}/me/drive/root:${encodeURI(folderPath)}`,
      { headers: h },
    );
    ...
    const parentData = parentRes.ok
      ? ((await parentRes.json()) as Record<string, unknown>)
      : null;
  ...
  private resolveParentEndpoint(parentId?: string): string {
    if (parentId) {
      return `${GRAPH_BASE}/me/drive/items/${parentId}`;
    }
    // Default to tenant root folder path
    return `${GRAPH_BASE}/me/drive/root:/NeogesysSport/${this.tenantSlug}:`;
  }
