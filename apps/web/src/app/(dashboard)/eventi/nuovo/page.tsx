...
import { z } from "zod";
import { ArrowLeft, Upload, CalendarDays } from "lucide-react";
...
const eventoSchema = z
  .object({
    ...
    quotaIscrizione: z.coerce
      .number()
      .min(0, "La quota non puo essere negativa")
      .optional(),
    maxPartecipanti: z.coerce
      .number()
      .int()
      .min(1, "Minimo 1 partecipante")
      .optional(),
    deadlineIscrizione: z.string().optional(),
  })
  .refine(
    (data) => !data.dataFine || data.dataFine >= data.dataInizio,
    {
      ...
      >
        <div className="rounded-xl border border-border bg-card p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            />
            {errors.luogo && (
              <p className="text-xs text-destructive">{errors.luogo.message}</p>
            )}
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              {errors.quotaIscrizione && (
                <p className="text-xs text-destructive">
                  {errors.quotaIscrizione.message}
                </p>
              )}
            </div>
            <div className="space-y-2">
              ...
              {errors.maxPartecipanti && (
                <p className="text-xs text-destructive">
                  {errors.maxPartecipanti.message}
                </p>
              )}
            </div>
          </div>
