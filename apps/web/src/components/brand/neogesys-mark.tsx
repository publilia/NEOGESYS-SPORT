/**
 * NEOGESYS · Brand mark (quasar + wordmark)
 * ─────────────────────────────────────────────────────────────────
 * Set logo ufficiale NEOGESYS consegnato da Claude Design il
 * 2026-04-19. Sostituisce in modo permanente il monogramma "N"
 * usato fino alla 2026.4.0 e il wordmark disegnato a mano usato
 * fino alla 2026.4.1.
 *
 * Componenti disponibili:
 *
 *   <NeogesysMark />
 *     Quasar completa — nucleo luminoso, disco di accrescimento
 *     viola→magenta→viola, doppio getto polare asimmetrico, quark
 *     cyan sul bordo frontale del disco. Colori hard-coded → la
 *     brand identity non cambia al variare del tema del parent.
 *     Da usare per favicon, splash, app icon, stampati, social og.
 *
 *   <NeogesysMarkMinimal />
 *     Stessa quasar ma in `currentColor` e senza il quark cyan.
 *     Da usare nel quasar-btn del QuasarMenu (icona pulsante in
 *     alto a destra del sidebar) e ovunque serva la variante
 *     mono-cromatica che si adatta al parent (es. icon button).
 *
 *   <NeogesysLogoHorizontal />
 *     Logotipo orizzontale compatto = quasar + wordmark "NEOGESYS".
 *     Il wordmark usa `currentColor` → il parent decide la tinta
 *     (Starlight #FFF6D9 su fondo scuro #13102A nell'AppHeader,
 *     nero su bianco nei documenti PDF). ViewBox 260×40.
 *     Usato nell'AppHeader a sinistra con accanto la label del
 *     settore (SPORT, PHARMA, RETAIL, …) come pill separata.
 *
 *   <NeogesysLogoFull />
 *     Logotipo completo = quasar + wordmark + tagline "Universal
 *     Suite". ViewBox 260×48. Usato nella splash/login/marketing
 *     dove la tagline diventa leggibile (≥ 32px di altezza).
 *     Il tagline usa `currentColor` con opacity=.55 per emularne
 *     l'effetto "secondario" del file originale (#8A7F99 light /
 *     #9B90B5 dark) restando tematico.
 *
 *   <NeogesysWordmark />
 *     Solo il wordmark "NEOGESYS" (senza quasar e senza tagline).
 *     Per situazioni in cui la mark è già presente altrove (es.
 *     email header con favicon già visibile, footer con pittogramma
 *     compagno).
 *
 * Tutti i componenti sono 1:1 con i rispettivi file SVG in
 * /public/brand/ (neogesys-mark.svg, neogesys-mark-minimal.svg,
 * neogesys-logo-horizontal.svg, neogesys-wordmark.svg). Se uno
 * cambia, aggiornare anche l'altro — o preferibilmente rigenerarli
 * dal logo set ufficiale in design-tokens.
 *
 * Gli id dei <defs> hanno suffissi univoci (`-nm`, `-nmm`, `-nlh`,
 * `-nlf`) per evitare collisioni quando più varianti coesistono
 * sulla stessa pagina (es. NeogesysMark nell'AI Assistant drawer
 * mentre NeogesysLogoHorizontal è nel header).
 */

interface MarkProps {
	className?: string;
	/** Dimensione. Default: eredita dal container (100%). */
	size?: number | string;
	/** Accessible title. Default: "NEOGESYS". */
	title?: string;
	/** Se true, la mark non annuncia un titolo al reader (decorativa). */
	ariaHidden?: boolean;
}
...
export function NeogesysMark({
	...
	return (
		...
		>
			...
			<defs>
				<radialGradient id="quasar-core-nm" cx=".5" cy=".5" r=".5">
				...
				<linearGradient id="quasar-jet-nm" x1=".5" y1="0" x2=".5" y2="1">
				...
				<linearGradient id="quasar-disk-nm" x1="0" y1=".5" x2="1" y2=".5">
					<stop offset="0%" stopColor="#8B3FE5" />
					<stop offset="50%" stopColor="#ED3F9E" />
					<stop offset="100%" stopColor="#8B3FE5" />
				</linearGradient>
			</defs>
			<path d="M15.6 15 Q15.9 9 16 2 Q16.1 9 16.4 15 Z" fill="url(#quasar-jet-nm)" />
			<path
				d="M15.6 17 Q15.9 22 16 28 Q16.1 22 16.4 17 Z"
				fill="url(#quasar-jet-nm)"
				opacity=".9"
			/>
			<ellipse
				cx="16"
			...
			/>
			<circle cx="16" cy="16" r="3.6" fill="url(#quasar-core-nm)" />
...
// ────────────────────────────────────────────────────────────────────
// NeogesysMarkMinimal — mono-cromatico (currentColor), no quark cyan
// ────────────────────────────────────────────────────────────────────
...
export function NeogesysMarkMinimal({
	...
	return (
		...
		>
			{!ariaHidden ? <title>{title}</title> : null}
			<defs>
				<radialGradient id="quasar-core-nmm" cx=".5" cy=".5" r=".5">
					<stop offset="0%" stopColor="currentColor" />
					<stop offset="35%" stopColor="currentColor" stopOpacity=".9" />
					<stop offset="100%" stopColor="currentColor" stopOpacity="0" />
				</radialGradient>
				<linearGradient id="quasar-jet-nmm" x1=".5" y1="0" x2=".5" y2="1">
					<stop offset="0%" stopColor="currentColor" stopOpacity="0" />
					<stop offset="50%" stopColor="currentColor" stopOpacity=".9" />
					<stop offset="100%" stopColor="currentColor" />
				</linearGradient>
			</defs>
			<path d="M15.6 15 Q15.9 9 16 2 Q16.1 9 16.4 15 Z" fill="url(#quasar-jet-nmm)" />
			<path
				d="M15.6 17 Q15.9 22 16 28 Q16.1 22 16.4 17 Z"
				fill="url(#quasar-jet-nmm)"
				opacity=".9"
			/>
			<ellipse
				cx="16"
			...
			<ellipse
				...
				strokeDasharray=".7 1.1"
			/>
			<circle cx="16" cy="16" r="3.6" fill="url(#quasar-core-nmm)" />
...
// ────────────────────────────────────────────────────────────────────
// NEOGESYS wordmark paths — source of truth
// ────────────────────────────────────────────────────────────────────
// Glifi tipografici ufficiali (fontforge → SVG path), estratti dal
// canonical neogesys-logo-horizontal.svg. Letter height 25.87 unit,
// viewBox-local baseline a y=0. Usati da NeogesysLogoHorizontal,
// NeogesysLogoFull e NeogesysWordmark — unica fonte per evitare
// drift tra le tre varianti.
...
// ────────────────────────────────────────────────────────────────────
// Quasar inline (DRY) — usato da Horizontal e Full
// ────────────────────────────────────────────────────────────────────
...
function QuasarInline({ idSuffix }: { idSuffix: string }) {
	return (
		<>
			<defs>
				<radialGradient id={`qc-${idSuffix}`} cx=".5" cy=".5" r=".5">
					<stop offset="0%" stopColor="#FFF6D9" />
					<stop offset="35%" stopColor="#FFF6D9" stopOpacity=".9" />
					<stop offset="100%" stopColor="#FFF6D9" stopOpacity="0" />
				</radialGradient>
				<linearGradient id={`qj-${idSuffix}`} x1=".5" y1="0" x2=".5" y2="1">
					<stop offset="0%" stopColor="#FFF6D9" stopOpacity="0" />
					<stop offset="50%" stopColor="#FFF6D9" stopOpacity=".9" />
					<stop offset="100%" stopColor="#FFF6D9" />
				</linearGradient>
				<linearGradient id={`qd-${idSuffix}`} x1="0" y1=".5" x2="1" y2=".5">
					<stop offset="0%" stopColor="#8B3FE5" />
					<stop offset="50%" stopColor="#ED3F9E" />
					<stop offset="100%" stopColor="#8B3FE5" />
				</linearGradient>
			</defs>
			<path d="M15.6 15 Q15.9 9 16 2 Q16.1 9 16.4 15 Z" fill={`url(#qj-${idSuffix})`} />
			<path
				d="M15.6 17 Q15.9 22 16 28 Q16.1 22 16.4 17 Z"
				fill={`url(#qj-${idSuffix})`}
				opacity=".9"
			/>
			<ellipse
				cx="16"
			...
			<ellipse
				...
				strokeDasharray=".7 1.1"
			/>
			<circle cx="16" cy="16" r="3.6" fill={`url(#qc-${idSuffix})`} />
			<circle cx="16" cy="16" r="1.1" fill="#FFF6D9" />
			<circle cx="22.4" cy="19.4" r=".95" fill="#3BD0F2" />
		</>
	);
...
// ────────────────────────────────────────────────────────────────────
// NeogesysLogoHorizontal — quasar + wordmark (senza tagline)
// ────────────────────────────────────────────────────────────────────
// ViewBox 260×40: quasar a sinistra (translate 4 4), wordmark
// "NEOGESYS" a destra (translate 44 34). Il wordmark usa
// currentColor → il parent sceglie la tinta.
//
// Uso canonico: AppHeader con wrapper #13102A + color #FFF6D9,
// dove accanto compare la pill del sector (SPORT). Ma il
// componente è theme-agnostic.
...
// ────────────────────────────────────────────────────────────────────
// ViewBox 260×48: include la tagline della suite umbrella. Da
// usare nella splash/login/marketing a ≥ 32px di altezza — più
// piccolo la tagline diventa illeggibile (cap height ~9.88 → a
// 20px di altezza sarebbe ~4px, OK solo per accent).
...
// ────────────────────────────────────────────────────────────────────
// NeogesysWordmark — solo lettere "NEOGESYS" (senza quasar, senza tagline)
// ────────────────────────────────────────────────────────────────────
// ViewBox 212×30: cap height 25.87, padding laterale minimo per
// allineamento. Usato quando la mark è già presente altrove e
// servono solo le lettere (es. email header, footer a riga, token
// di brand in un chip).
