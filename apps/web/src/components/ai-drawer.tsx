...
import { Bot, Check, Send, Sparkles, User, X, XCircle } from "lucide-react";
	...
	const [messages, setMessages] = useState<Message[]>([
		{
			id: "welcome",
			role: "assistant",
			content: "Ciao! Sono il tuo assistente AI per la gestione sportiva. Come posso aiutarti?",
		},
	]);
						...
						<div className="flex h-14 items-center justify-between border-b border-border px-4">
							<div className="flex items-center gap-2">
								...
								<div>
									...
									<p className="text-[10px] text-muted-foreground">Assistente gestionale</p>
								</div>
							</div>
							<button
								type="button"
											...
											<div className="ml-10 mt-2 rounded-lg border border-border bg-card p-3">
												...
												{message.toolCall.status === "pending" && (
													<div className="mt-2 flex gap-2">
														<button
															type="button"
															onClick={() => handleToolAction(message.id, "confirmed")}
														...
														<button
															type="button"
															onClick={() => handleToolAction(message.id, "cancelled")}
												...
												{message.toolCall.status === "confirmed" && (
													<p className="mt-2 text-xs text-green-600">Azione confermata</p>
												)}
												{message.toolCall.status === "cancelled" && (
													<p className="mt-2 text-xs text-destructive">Azione annullata</p>
												)}
											</div>
										)}
									</div>
