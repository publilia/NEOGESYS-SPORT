...
const createTenantInput = z.object({
  ...
  federazioni: z
    .array(z.object({ nome: z.string(), codice: z.string().optional() }))
    .optional(),
});
...
export const tenantRouter = router({
  ...
  overview: superAdminProcedure.query(async ({ ctx }) => {
    const [totals, byStato, byPiano] = await Promise.all([
      ...
      ctx.db
        .select({ stato: tenants.stato, count: count() })
        .from(tenants)
        .groupBy(tenants.stato),
      ctx.db
        .select({ piano: tenants.piano, count: count() })
        .from(tenants)
        .groupBy(tenants.piano),
    ]);
