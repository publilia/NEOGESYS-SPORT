...
type SimilarResult = { socioId: string; testoOriginale: string; similarita: number };
		...
		for (const row of sociRows) {
			try {
				const features = extractFeatures({
					...
					ultimaQuotaPagata: row.ultima_quota_pagata
						? new Date(String(row.ultima_quota_pagata))
						: null,
	...
	query: protectedProcedure
      ...
      `);
			...
			if (client.provider !== "anthropic" || !client.anthropic) {
				return {
					risposta:
						"Il modulo AI non è configurato per questo tenant. Aggiungi una chiave API dalle impostazioni.",
					fonti: [],
				};
			}
...
${contesto ? `\nContesto soci rilevanti:\n${contesto}` : ""}`;
			...
			const textBlock = response.content.find((b: ContentBlock) => b.type === "text");
			const risposta =
				textBlock?.type === "text" ? textBlock.text : "Impossibile generare una risposta.";
	...
	processDocument: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const response = await client.anthropic.messages.create({
				...
				max_tokens: 1024,
				system:
					"Estrai i dati principali dal documento. Restituisci un JSON con i campi trovati. Solo JSON, nessun markdown.",
				messages: [
					{
						role: "user",
						content: [
			...
			if (!textBlock || textBlock.type !== "text") {
				throw new TRPCError({
					...
					message: "Nessuna risposta dal modello OCR.",
				});
			}
...
Rispondi SOLO con un JSON: { "oggetto": "...", "testo": "..." }. Niente markdown.`,
	...
	searchSemantic: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const socioIds = similarResults.map((r: SimilarResult) => r.socioId);
      ...
      `);
			...
			return {
				results: similarResults.map((r: SimilarResult) => {
