...
const listInput = z.object({
  ...
  tipologia: z
    .enum(["socio", "atleta", "istruttore", "dirigente", "volontario"])
    .optional(),
  search: z.string().optional(),
  sortBy: z
    .enum(["cognome", "nome", "dataIscrizione", "createdAt"])
    .default("cognome"),
...
export const sociRouter = router({
  ...
  list: protectedProcedure.input(listInput).query(async ({ ctx, input }) => {
    ...
    const { page, perPage, stato, disciplina, tipologia, search, sortBy, sortOrder } =
      input;
  ...
  create: protectedProcedure.input(createSocioInput).mutation(async ({ ctx, input }) => {
    ...
    const [socio] = await ctx.db
      ...
      .values({
        ...
        dataIscrizione: input.dataIscrizione
          ? new Date(input.dataIscrizione)
          : new Date(),
        note: input.note,
      })
      .returning();
