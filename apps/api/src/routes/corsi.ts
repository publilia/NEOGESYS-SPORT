...
export const corsiRouter = router({
  ...
  update: protectedProcedure.input(updateCorsoInput).mutation(async ({ ctx, input }) => {
    ...
    if (data.maxPartecipanti !== undefined) setClauses.push(sql`max_partecipanti = ${data.maxPartecipanti}`);
    ...
    if (data.orario !== undefined) setClauses.push(sql`orario = ${JSON.stringify(data.orario)}::jsonb`);
