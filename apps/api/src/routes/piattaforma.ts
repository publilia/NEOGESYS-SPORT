import { fatturePiattaforma, planiAbbonamento, superAdminAuditLog, tenants } from "@neogesys/db";
import { TRPCError } from "@trpc/server";
import { and, count, desc, eq, gte, sql } from "drizzle-orm";
import { z } from "zod";
import { protectedProcedure, router, superAdminProcedure } from "../trpc/index";
...
export const pianiRouter = router({
	...
	update: superAdminProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (data.prezzoMensile !== undefined)
				updateData.prezzoMensile = data.prezzoMensile.toString();
			if (data.prezzoAnnuale !== undefined)
				updateData.prezzoAnnuale = data.prezzoAnnuale.toString();
...
export const fattureRouter = router({
	...
	revenue: superAdminProcedure.query(async ({ ctx }) => {
		const [mrr] = await ctx.db
			...
			.where(
				and(
					...
					gte(fatturePiattaforma.dataEmissione, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)),
				),
			);
		...
		return {
			mrr: Number.parseFloat(mrr?.totale ?? "0"),
			pendingRevenue: Number.parseFloat(pending?.totale ?? "0"),
		};
	}),
});
	...
	list: superAdminProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			if (input.superAdminId)
				conditions.push(eq(superAdminAuditLog.superAdminId, input.superAdminId));
			const where = conditions.length > 0 ? and(...conditions) : undefined;
