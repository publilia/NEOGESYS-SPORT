import { MicrosoftOAuthClient, ONEDRIVE_SCOPES } from "@neogesys/integrations/microsoft";
...
export const microsoftOneDriveRouter = router({
	...
	handleCallback: adminProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			await ctx.vault.setCredentials(tenantId, "microsoft_onedrive", "storage", {
				clientId: input.clientId,
				clientSecret: input.clientSecret,
	...
	shareFile: adminProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const response = await graphFetch(client, `/me/drive/items/${input.fileId}/invite`, {
				method: "POST",
				body: JSON.stringify({
					requireSignIn: true,
					sendInvitation: true,
