...
export default function PortaleLoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    setIsLoading(true);
    try {
      // TODO: Replace with actual portale-socio API call
      console.log("Login socio attempt:", data.email);
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-md space-y-8">
        ...
        <p className="text-center text-xs text-muted-foreground">
          Le credenziali di accesso ti sono state fornite dalla segreteria
          della tua associazione sportiva.
        </p>
      </div>
    </div>
  );
