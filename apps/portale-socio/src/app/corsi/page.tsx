...
export default function CorsiPage() {
	const presenzeRate =
		presenze.length > 0
			? Math.round((presenze.filter((p) => p.presente).length / presenze.length) * 100)
			: 0;
		...
		<div className="space-y-8">
			...
			<div>
				...
				<div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
					{corsiIscritti.map((corso) => (
						<div key={corso.id} className="rounded-xl border border-border bg-card p-5 shadow-sm">
							...
							<p className="mt-1 text-sm text-muted-foreground">Istruttore: {corso.istruttore}</p>
					...
					<table className="w-full text-sm">
						<thead>
							<tr className="border-b border-border">
								<th className="px-4 py-3 text-left font-medium text-muted-foreground">Giorno</th>
								...
								<th className="px-4 py-3 text-left font-medium text-muted-foreground">Luogo</th>
							</tr>
						</thead>
						<tbody className="divide-y divide-border">
							{giorniSettimana.map((giorno) => {
								const corsiGiorno = corsiIscritti.filter((c) => c.giorno.includes(giorno));
								...
								return corsiGiorno.map((corso, idx) => (
									<tr key={`${giorno}-${corso.id}`} className="hover:bg-accent/30">
										...
										<td className="px-4 py-3 text-muted-foreground">{corso.orario}</td>
										<td className="px-4 py-3 text-muted-foreground">{corso.luogo}</td>
									</tr>
								));
							})}
						</tbody>
			...
			<div>
				<div className="flex items-center justify-between">
					<h2 className="text-lg font-semibold text-foreground">Storico presenze</h2>
