...
import { ArrowLeft, ChevronRight, Clock, Mail, MailOpen, Megaphone } from "lucide-react";
...
const messaggi: Messaggio[] = [
	{
		...
		anteprima: "Si informa che a partire dalla prossima settimana la lezione di Nuoto Avanzato...",
		contenuto:
	...
	},
	{
		...
		anteprima: "Ti ricordiamo che la quota mensile di Aprile e in scadenza il 30/04...",
		contenuto:
	...
	},
	{
		...
		anteprima: "Sono aperte le iscrizioni per il Torneo Interclubs che si terra il 15 maggio...",
		contenuto:
	...
	},
	{
		...
		mittente: "Segreteria ASD",
		anteprima: "Si comunica che gli impianti sportivi resteranno chiusi dal 1 al 31 agosto...",
		contenuto:
...
export default function ComunicazioniPage() {
	...
	const messaggiFiltrati =
		filtro === "tutti" ? messaggi : messaggi.filter((m) => m.tipo === filtro);
			...
			<div className="space-y-6">
				...
				<div className="rounded-xl border border-border bg-card p-6 shadow-sm">
					<div className="flex items-start justify-between">
						<div>
							<h2 className="text-xl font-semibold text-foreground">{selectedMessage.oggetto}</h2>
							<p className="mt-1 text-sm text-muted-foreground">Da: {selectedMessage.mittente}</p>
						</div>
				...
				<div className="divide-y divide-border">
								...
								<div className="flex items-center gap-2">
									<p
										className={`text-sm truncate ${
											msg.letto ? "text-foreground" : "font-semibold text-foreground"
										}`}
									>
										{msg.oggetto}
									</p>
									{!msg.letto && <span className="h-2 w-2 shrink-0 rounded-full bg-primary" />}
								</div>
					...
					{messaggiFiltrati.length === 0 && (
						<div className="px-5 py-10 text-center">
							...
							<p className="mt-2 text-sm text-muted-foreground">Nessuna comunicazione trovata</p>
						</div>
					)}
				</div>
			</div>
