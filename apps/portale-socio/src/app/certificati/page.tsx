...
function calcolaGiorniRimanenti(dataScadenza: string): number {
	...
	const scadenza = new Date(anno ?? 0, (mese ?? 1) - 1, giorno ?? 1);
	const oggi = new Date();
...
export default function CertificatiPage() {
	...
	return (
		<div className="space-y-8">
				...
				{certificati.map((cert) => {
					...
					return (
						<div key={cert.id} className="rounded-xl border border-border bg-card p-5 shadow-sm">
							...
							<p className="mt-1 text-sm text-muted-foreground">{cert.medicoRilasciante}</p>
							...
							<div className="mt-4 rounded-lg bg-muted/50 p-3">
								<div className="flex items-center gap-2 text-sm">
									...
									<span className="font-medium text-foreground">{cert.dataScadenza}</span>
								</div>
								{cert.stato !== "scaduto" ? (
									<p className="mt-1 text-xs text-muted-foreground">
										...
										<span
											className={`font-semibold ${
												giorniRimanenti <= 30 ? "text-amber-600" : "text-foreground"
											}`}
										>
									...
									<p className="mt-1 text-xs font-medium text-destructive">
										Scaduto da {Math.abs(giorniRimanenti)} giorni - rinnovo necessario
									</p>
								)}
							</div>
			...
			<div>
				<h2 className="text-lg font-semibold text-foreground">Carica nuovo certificato</h2>
				<div
				...
				>
					...
					<p className="mt-3 text-sm font-medium text-foreground">Trascina il file qui oppure</p>
					<button
						type="button"
					...
					<p className="mt-2 text-xs text-muted-foreground">PDF, JPG o PNG, massimo 10 MB</p>
				</div>
			</div>
