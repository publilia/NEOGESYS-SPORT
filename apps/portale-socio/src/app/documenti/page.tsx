...
export default function DocumentiPage() {
	...
	const filteredDocs = documenti.filter(
		(doc) =>
		...
		<div className="space-y-8">
			...
			<div className="rounded-xl border border-border bg-card shadow-sm">
				<div className="divide-y divide-border">
					{filteredDocs.map((doc) => (
						<div key={doc.id} className="flex items-center gap-4 px-5 py-4 hover:bg-accent/30">
					...
					{filteredDocs.length === 0 && (
						<div className="px-5 py-10 text-center">
							...
							<p className="mt-2 text-sm text-muted-foreground">Nessun documento trovato</p>
						</div>
					)}
				</div>
			</div>
			...
			<div>
				...
				>
					...
					<p className="mt-2 text-xs text-muted-foreground">PDF, JPG, PNG o DOC, massimo 10 MB</p>
				</div>
			</div>
		</div>
	);
